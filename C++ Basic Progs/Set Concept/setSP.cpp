...
private:
	int num,flag=1;
public:
	    list<int>l,l1,u,I,d,ss,temp;
	    list<int>::iterator t,t1,t2,t3,t4,t5;
void add();
void delete1(int);
void search(int);
void searchB(int);
...
void set::Sub()
{
    int n;
	int flag=0;
	cout<<"How many Elements are in the Subset:\n";
	cin>>n;
	cout<<"Enter Elements: \n";
		for(int i=0;i<n;i++)
		{
	...
	for(t=l.begin();t!=l.end();t++)
		{
			for(t1=temp.begin();t1!=temp.end();*t1++)
			{
				if(*t==*t1)
					{
						ss.push_back(*t1);
    ...
    for(t=temp.begin();t!=temp.end();t++)
		...
		if(ss.empty())
			...
			cout<<"The Subset is present in Set A : \t";
}
